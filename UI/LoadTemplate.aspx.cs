                    ...
                    totalSupplierCell == supplierTotal)
                {
                    for (int row1 = 1; row1 < table.Rows.Count; row1++)
                    {
                        ...
                        if (result < 0)
                        {
                            ...
                            invoiceSupplierManager.InsertInvoiceSupplier(new InvoiceSupplier(table.Rows[row1][4].ToString(), table.Rows[row1][9].ToString(), DateTime.Now, 0, "", double.Parse(table.Rows[row1][23].ToString()), 0, DateTime.ParseExact(table.Rows[row1][3].ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture)));
                        }
                        else
                        {
                            //relationship = "is later than";
